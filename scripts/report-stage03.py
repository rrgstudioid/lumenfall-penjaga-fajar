"""Generate evidence links and measured delivery report from the final local run."""
import json,hashlib,html,shutil
from pathlib import Path
root=Path(__file__).resolve().parents[1]
stage=root/'dev-prototypes/mahkota-fajar-stage03-v1';ev=stage/'evidence'
m=json.loads((stage/'assets/manifest.json').read_text());q=json.loads((ev/'final-browser.json').read_text());p=json.loads((stage/'provenance.json').read_text(encoding='utf-8-sig'))
assert hashlib.sha256(Path(p['source']).read_bytes()).hexdigest().upper()==p['sha256']
shutil.copyfile(p['reference'],ev/'reference-blueprint.png')
views=['top','overview','center','guild','merchant','training','job','forge','warp','player','street']
gallery=['<!doctype html><meta charset="utf-8"><title>LUMENFALL Stage03 comparison</title><style>body{background:#132026;color:#eedbbb;font:16px system-ui;margin:24px}img{max-width:100%}.pair{display:grid;grid-template-columns:1fr 1fr;gap:12px}figure{margin:0}h2{margin-top:40px}a{color:#acdfff}</style><h1>LUMENFALL Stage03 — development candidate</h1><p>Blueprint → working Blender → actual Game runtime. Runtime: 1920×1080, DPR 1. Blender previews: 1280×720. Matched 45-degree vertical FOV; different renderers and lighting, not a pixel-identical comparison.</p><img src="reference-blueprint.png" alt="Locked blueprint">']
for v in views:
    gallery.append('<h2>'+v+'</h2><div class="pair">')
    if (ev/('blender-'+v+'.jpg')).exists():gallery.append('<figure><figcaption>Working Blender</figcaption><img src="blender-'+v+'.jpg"></figure>')
    gallery.append('<figure><figcaption>Three.js / existing Game</figcaption><img src="runtime-'+v+'.png"></figure></div>')
gallery.append('<h2>Traversal destinations</h2>')
for v in ['warp','guild','merchant','training','job','forge','bank','inn','residential']:gallery.append('<h3>'+v+'</h3><img loading="lazy" src="destination-'+v+'.png">')
(ev/'comparison.html').write_text('\n'.join(gallery),encoding='utf-8')
metrics=q['reload'][-1]['state']['metrics'];routes=q['routes'];payload=m['payloadBytes']+(stage/'assets/manifest.json').stat().st_size
files=['lib/game/stage03-collision.ts','lib/game/stage03-collision.test.ts','tests/browser/mahkota-fajar-stage03-scene.ts','tests/browser/mahkota-fajar-stage03.ts','tests/browser/mahkota-fajar-stage03.html','tests/browser/mahkota-fajar-stage03.vite.config.ts']+[str(x.relative_to(root)).replace('\\','/') for x in (root/'scripts').glob('*stage03*')]
lines=['# LUMENFALL Stage03 — implementation and local validation','',
'Development candidate: `'+m['mapId']+'`. Menggunakan Game, player, movement, animasi, dan kamera proyek yang sama. Bukan pengganti map produksi; tidak dipublikasikan. Kandidat playable tersedia, tetapi kesetaraan artistik penuh dengan ilustrasi referensi **belum dapat dinyatakan lulus**.','',
'## Jalankan','',
'```powershell','node node_modules/vite/bin/vite.js --config tests/browser/mahkota-fajar-stage03.vite.config.ts','```','',
...
'## Sumber dan file','',
'Source asli: `'+p['source']+'`.',
'SHA256 awal dan verifikasi akhir sama: `'+p['sha256']+'`.',
...
lines += ['- `'+f+'`' for f in sorted(set(files+['docs/LUMENFALL-Stage03-Implementation.md']))]
lines += ['', 'Aset besar, Blender, dan bukti lokal berada di `dev-prototypes/mahkota-fajar-stage03-v1/` yang diabaikan Git. Keberadaan lokal tidak berarti aset tersimpan di commit/remote.', '',
...
'## Angka terukur','',
'| Ukuran | Hasil |','|---|---:|',
f"| Resident geometry lintas near/LOD/module (unik; bukan instance-expanded) | {metrics['triangles']:,} triangles |",
f"| Peak primary-pass triangles selama traversal (termasuk player) | {max(r['maxPrimaryTriangles'] for r in routes):,} |",
f"| Peak triangles termasuk shadow pass | {max(r['maxTriangles'] for r in routes):,} |",
f"| Peak draw calls termasuk shadow selama traversal | {max(r['maxDraws'] for r in routes)} |",
f"| Material map aktif | {metrics['materials']} |",
f"| Estimasi texture residency RGBA8+mipmaps | {metrics['textures']/1048576:.2f} MiB |",
f"| Payload transport + manifest | {payload:,} bytes / {payload/1048576:.2f} MiB |",
f"| GLB decoded, tidak termasuk PNG | {m['decodedAssetBytes']:,} bytes |",
f"| File runtime + manifest | {len(m['payloadFiles'])+1} |",
f"| P95 frame terburuk dari 18 rute | {max(r['frameP95'] for r in routes):.2f} ms |",'',
...
'Perangkat/browser: `'+q['checks']['hardware']['renderer']+'`; `'+q['checks']['hardware']['userAgent']+'`; 1920×1080 DPR 1, '+str(q['checks']['hardware']['cores'])+' logical cores.','',
'## Rute dua arah','',
...
for r in routes:lines.append(f"| {r['target']} | {'kembali' if r['reverse'] else 'pergi'} | {r['distance']:.2f} | {r['walkSeconds']:.2f}s | {r['minimumObstacleClearWidth']:.2f} | {r['frameP95']:.2f} | {r['pass']} |")
lines += ['', '## Validasi dan batas bukti','']
for k,v in q['checks'].items():
    if k!='hardware':lines.append('- `'+k+'`: `'+json.dumps(v)+'`.')
lines += [f"- Error/retry: errorDisplayed={q['errorDisplayed']}, retryPassed={q['retryPassed']}; page errors={q['errors']}.",
'- Tiga reload: '+str([(r['rootChildren'],r['canvases'],r['memory']) for r in q['reload']])+'. Stabilitas hitungan mendukung tidak ada duplikasi pada tiga siklus; bukan bukti heap leak jangka panjang.',
...
(root/'docs/LUMENFALL-Stage03-Implementation.md').write_text('\n'.join(lines),encoding='utf-8')
print(json.dumps(dict(sourceHashVerified=True,report='docs/LUMENFALL-Stage03-Implementation.md',payload=payload,metrics=metrics)))
