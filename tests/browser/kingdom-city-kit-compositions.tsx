// @ts-nocheck — isolated browser-only composition test entry.
...
const loader=new GLTFLoader();let proxy=null;
...
function clear(){while(root.children.length){const o=root.children.pop();o.traverse(n=>{const m=n;m.geometry?.dispose();const ms=Array.isArray(m.material)?m.material:m.material?[m.material]:[];ms.forEach(x=>x.dispose())})}}
async function load(id,pos){const a=await fetch(stage+'metadata/kit-manifest.json').then(r=>r.json());const d=a.assets.find(x=>x.assetId===id);if(!d||d.format!=='GLTF')throw new Error(`${id} is not renderable GLTF`);const g=await loader.loadAsync(d.viewerUrl);g.scene.position.set(...pos);g.scene.traverse(n=>{const m=n;m.castShadow=true;m.receiveShadow=true});root.add(g.scene);return d.displayName}
...
async function build(kind){
  ...
  if(kind==='fortification'){
    ...
    if(proxy){const p=proxy.clone();p.position.set(0,0,2.3);p.visible=document.getElementById('proxy').checked;root.add(p)}
    ...
    if(proxy){const p=proxy.clone();p.position.set(0,0,2);p.visible=document.getElementById('proxy').checked;root.add(p)}
    ...
    if(proxy){const p=proxy.clone();p.position.set(-6,0,0);p.visible=document.getElementById('proxy').checked;root.add(p)}
...
document.querySelectorAll('[data-scene]').forEach(b=>b.addEventListener('click',()=>build(b.dataset.scene)));
document.getElementById('proxy').addEventListener('change',()=>build('street'));
