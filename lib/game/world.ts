...
export class Game {
  ...
  constructor(
    ...
    this.scene.background = new T.Color('#9aaf8d');
    this.scene.fog = new T.Fog('#9aaf8d', 52, 108);
    this.scene.add(this.worldLightRig);
    this.scene.add(this.sandsLightRig);
    this.worldLightRig.add(new T.HemisphereLight('#e4eecc', '#385544', 1.9));
    ...
    const sandsHemisphere=new T.HemisphereLight('#fff7df','#7c6a4a',1.35);
    const sandsSun=new T.DirectionalLight('#fff1c7',1.65);
    sandsSun.position.set(-18,32,12);
  ...
  buildRegionDecor() {
    ...
    const color = this.hero.inCity ? this.hero.currentCity==='arunika'?'#536e49':'#424653' : field.color;
    this.scene.background = new T.Color(color); this.scene.fog = new T.Fog(color,52,108);
    ...
    if(isSandsLocation) {
      ...
      // Render the supplied GLB without fog tinting its terrain or textures.
      this.scene.background=new T.Color('#c8ad73');this.scene.fog=null;
    ...
    if(this.fieldTerrain) {
      ...
      this.scene.background=new T.Color('#b9d6c7');this.scene.fog=new T.Fog('#b9d6c7',65,160);
  ...
  move(dx: number, dz: number) {
    ...
    this.actor.position.set(p.x,this.groundHeight(p.x,p.z),p.z);
  }
  ...
  drawMap() {
    ...
    // Padang Arunika has a field-specific 2x horizontal footprint. Keep the
    // minimap transform local to that field so city and other maps retain the
    // existing projection.
    ...
    for (const e of this.enemies) {
      ...
      ctx.beginPath();
      ctx.arc(
        p(e.group.position.x),
        p(e.group.position.z),
        e.boss ? 4 : 2,
        0,
        Math.PI * 2,
      );
    ...
    ctx.save();
    ctx.translate(p(this.hero.x), p(this.hero.z));
