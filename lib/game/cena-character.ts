...
import { loadCenaLegacyMotion, loadCenaRunMotion } from './cena-motion.ts';
...
export async function loadCenaCharacter() {
  ...
  const [loaded, legacyMotion, runMotion] = await Promise.all([source, loadCenaLegacyMotion().catch(error => {
    console.warn('Animasi lama Cena gagal dimuat; gerak procedural tetap tersedia.', error);
    return [];
  }), loadCenaRunMotion().catch(error => {
    console.warn('Run F0 gagal dimuat; animasi lari sebelumnya tetap tersedia.', error);
    return [];
  })]);
  const result = cloneCharacterSource(loaded.scene);
  applyCenaChinContour(result);
  softenCenaNeckWeights(result);
  ...
  // Ignore embedded actions. Only the separately approved animation packs run:
  // old Walk/attacks + Cena Run F0 Start/Loop/Stop (legacy Run is fallback only).
  result.userData.cenaSourceAnimationsDisabled = true;
  result.userData.lumenfallAnimations = runMotion.length ? [...legacyMotion.filter(c => c.name !== 'Run'), ...runMotion] : legacyMotion;
  result.userData.runningAnimationSource = runMotion.length ? 'cena-run-f0' : 'army-man-running-blender';
  result.userData.motionSource = 'legacy-walk-attacks+cena-run-f0';
  return result;
}
