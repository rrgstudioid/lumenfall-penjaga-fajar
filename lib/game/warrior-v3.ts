...
export const WARRIOR_V3_SKILLS: readonly SkillDefinitionV3[] = [
  ...
  makeDamage({ id: 'v3-warrior-sweeping-slash', name: 'Sweeping Slash', maxRank: 8, rankLevelRequirements: [20,24,29,34,40,46,51,56], damageProfile: { physicalCoefficient: .88, statScaling: { str: .08 } }, resourceCost: { mana: 8 }, cooldown: 6, targeting: { targetType: 'frontal_arc', range: 4.8, radius: 4.0, maxTargets: 3 }, prerequisiteSkills: [{ skillId: 'v3-warrior-strike', requiredRank: 2 }], effects: { tags: ['arc:120','no-knockback'] }, presentation: { description: 'Swing your sword in a wide horizontal arc, striking multiple enemies in front of you.' }, motion: motion('HORIZONTAL_SWEEP','Wide horizontal sword sweep.',['no full 360 spin','no jump','no knockback','no ground slam']) }),
  {
    id: 'v3-warrior-guard-stance', name: 'Guard Stance', jobId: 'warrior', jobTier: 'core', maxRank: 5, rankLevelRequirements: [22,30,38,46,54], spCostPerRank: 2, skillType: 'STANCE', weaponRequirement: swords, targeting: { targetType: 'self' }, resourceCost: { mana: 10 }, cooldown: 18, effects: { buffs: ['damageReduction','blockRate'] }, presentation: { description: 'Assume a defensive sword stance, reducing incoming damage and improving your ability to block attacks for a short time.' }, motion: motion('DEFENSIVE_GUARD','Defensive sword guard.',['no attack','no giant shield bubble','no immobility requirement','no offensive damage'])
  },
  makeDamage({ id: 'v3-warrior-armor-breaker', name: 'Armor Breaker', maxRank: 5, rankLevelRequirements: [25,33,41,48,55], damageProfile: { physicalCoefficient: 1, statScaling: { str: .10 } }, resourceCost: { mana: 11 }, cooldown: 10, prerequisiteSkills: [{ skillId: 'v3-warrior-strike', requiredRank: 3 }], effects: { debuffs: ['armor_break'], tags: ['refresh-strongest-only'] }, presentation: { description: "Deliver a forceful strike against the enemy's guard, weakening its Defense for a short duration." }, motion: motion('HEAVY_GUARD_BREAK','Forceful strike against the enemy guard.',['no knockback']) }),
  {
    id: 'v3-warrior-battle-cry', name: 'Battle Cry', jobId: 'warrior', jobTier: 'core', maxRank: 5, rankLevelRequirements: [28,35,42,49,56], spCostPerRank: 2, skillType: 'ACTIVE_BUFF', targeting: { targetType: 'self' }, resourceCost: { mana: 14 }, cooldown: 35, effects: { buffs: ['physicalDamagePercent'] }, presentation: { description: 'Release a powerful battle cry, temporarily increasing your physical offensive strength.' }, motion: motion('POWER_BUFF','Battle cry power-up pose.',['no damage','no sword attack','no giant magical explosion'])
  },
...
const adapter = (definition: SkillDefinitionV3): SkillDefinition => {
  ...
  const buffRankEffects = definition.id === 'v3-warrior-battle-cry'
    ? [3,4,5,6,7].map((value, index) => ({ temporaryBuffs: [{ duration: [20,22,24,26,28][index], modifier: { id: `${definition.id}-buff`, stats: { percent: { physicalAttack: value } } } }] }))
