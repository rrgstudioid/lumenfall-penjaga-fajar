...
await test('Model B resolves Accuracy against Evasion with one roll', () => {
  const cases = [
    [105, 1.5, 1.5, 0, 1],
    [105, 5.5, 1.5, 4, .96],
    [145, 5.5, 5.5, 0, 1],
...
await test('Model B clamps effective Evasion without adding an Accuracy cap', () => {
...
await test('clean Accuracy/Evasion fixture uses actual V3 skill, impact, status, Stun and Flow/Tempo paths', () => {
  ...
  assert.equal(result.twinOneLands.hits.filter((hit) => hit.result === 'HIT').length, 1);
  assert.equal(result.twinOneLands.tempoAfter, 1);
  assert.equal(result.twinOneLands.flowActiveAfter, false);
  assert.equal(result.twinBothEvade.hits.filter((hit) => hit.result === 'HIT').length, 0);
  assert.equal(result.twinBothEvade.tempoAfter, 0);
  assert.equal(result.twinBothEvade.flowActiveAfter, true);

  assert.equal(result.piercingNoOwner.hits.filter((hit) => hit.result === 'HIT').length, 0);
  assert.equal(result.piercingOtherOwner.hits.filter((hit) => hit.result === 'HIT').length, 0);
  assert.equal(result.piercingOwnOwner.hits.filter((hit) => hit.result === 'HIT').length, 3);

  assert.equal(result.tempest.hits.filter((hit) => hit.result === 'HIT').length, 3);
