...
import { BLADE_MASTER_V3_RUNTIME_SKILLS, BLADE_MASTER_V3_SKILL_MAP, BLADE_MASTER_DUAL_MANA_SKILLS, bladeMasterV3StateAfterSpecialization, bladeMasterDualWieldActive, composeBladeWeaponHits } from './blade-master-v3.ts';
...
export function resolveHeroSkill(hero:Hero, skill:SkillDefinition, rank=hero.skillLevels[skill.id]??1, stats=derivedStats(hero),counter?:CounterContext,extraModifiers:CombatModifier[]=[],impact?:ImpactContext,bladeTempoManaReduction=0) {
  ...
  const bladeManaReduction = bladeMasteryRank > 0 && dualSkill && weaponStyle === 'dual_sword' ? [0,0,2,4,6,8][Math.min(5, bladeMasteryRank)] : 0;
